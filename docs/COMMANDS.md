...
A queue governs one branch, so only the bottom layer targets it; the layer above becomes eligible only
once the entry lands and GitHub retargets it. That landing is on the queue's schedule, not when checks
pass - and it leaves the layer merged on the platform but still recorded locally, so `sync` has to clear
it before `merge --all` will carry on. The summary counts the queued review separately rather than
reporting a run that did everything available to it as one that merged nothing.
